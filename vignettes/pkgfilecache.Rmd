...
`ensure_files_available` downloads missing files in parallel by default, using 2 simultaneous connections, which can speed up downloading many small files. You can control this per call with the `num_connections` argument (use `num_connections=1` for strictly sequential downloads), or change the default for all calls with the R option `options(pkgfilecache.num_connections = N)`, which is useful, e.g., for users who want to make packages download their optional data faster.
