...
Technically, this package permanently stores the files in a subdirectory of the directory returned by `tools::R_user_dir(yourpackage, "data")` (for R version 4.0 or later), which is the location recommended by the CRAN repository policy for user-specific data and cache files. For my Ubuntu Linux system, that is `/home/myuser/.local/share/R/yourpackage`, but the exact location is operating system-dependent and you should not care about it. On R versions before 4.0, and if a cache from an older version of this package already exists at the legacy location, the directory returned by `rappdirs::user_data_dir` is used instead, so existing downloads are kept.

You can also control the cache location with R options. Setting `options(pkgfilecache.cachedir = "/some/dir")` places the cache under `/some/dir` (the package name and optional version are appended). Setting `options(pkgfilecache.use_tempdir = TRUE)` places the cache in a subdirectory of the R session's temporary directory, which is useful for unit tests and CI systems that must not write to the user's home directory.
...
`ensure_files_available` downloads missing files in parallel by default, using 2 simultaneous connections, which can speed up downloading many small files. You can control this per call with the `num_connections` argument (use `num_connections=1` for strictly sequential downloads), or change the default for all calls with the R option `options(pkgfilecache.num_connections = N)`, which is useful, e.g., for users who want to make packages download their optional data faster. Downloads that fail are retried automatically (up to `num_retries` times, default 2) using fresh connections, so transient connection problems do not cause files to be reported as missing.
...
### Declarative file manifests

Maintaining the three parallel vectors (`local_filenames`, `urls`, `md5sums`) is fine for a handful of files, but it gets tedious once your package has many optional data files. For this case, pkgfilecache lets you describe all files in a single declarative *manifest*, one row per file. A manifest is either a data.frame or a CSV file, with the following columns:

* `path` (required): the file path relative to the package cache, using `/` as separator. Subdirectories are created automatically.
* `url` (optional): the download URL. If empty, it is derived as `paste0(base_url, path)` from the `base_url` argument, which is convenient when the remote directory layout mirrors the layout inside the package cache.
* `md5` (optional): the MD5 checksum of the file. If empty, only the existence of the file is checked, not its integrity.

Use `ensure_files_available_from_manifest()` exactly like `ensure_files_available()`, but pass a manifest instead of three parallel vectors:
...
res = ensure_files_available_from_manifest(pkg_info, manifest, base_url = "https://your.server/yourpackage/");
```

A manifest can also be a CSV file, which is convenient to ship in your package and to edit without touching R code. Comment lines starting with `#` are ignored:

```{r, eval = FALSE}
manifest_path = system.file("extdata", "files.csv", package = "yourpackage");
res = ensure_files_available_from_manifest(pkg_info, manifest_path, base_url = "https://your.server/yourpackage/");
```

You can inspect or validate a manifest at any time with `read_manifest()`:

```{r, eval = FALSE}
manifest = read_manifest(manifest_path);   # validates the columns and the MD5 checksums
```

#### Generating a manifest from a local directory

Computing MD5 sums by hand is usually the most tedious part of adding files. `write_manifest_from_dir()` does this for you: it walks a local directory, computes the MD5 checksum of every file, and writes a ready-to-use manifest as a CSV file. Put your files into a directory (subdirectories are preserved), run it once, and add the resulting CSV to your package:

```{r, eval = FALSE}
# Put all your optional data files into a directory, then:
write_manifest_from_dir(dir = "~/yourpackage_data", out = "inst/extdata/files.csv",
                        url_base = "https://your.server/yourpackage/");
# If url_base is given, the 'url' column is filled with url_base + path for every file.
# If it is left NULL, the 'url' column stays empty and you pass base_url at download time.
```
...
If your package has many optional data files, you can implement `download_optional_data()` much more compactly using a declarative manifest instead of the three parallel vectors (see the section 'Declarative file manifests' above). The example below uses a data.frame as the manifest; you can just as well use a CSV file shipped with your package and pass its path instead:

```{r, eval = FALSE}
#' @title Download optional data for this package if required.
#'
#' @description Ensure that the optional data is available locally in the package cache. Will try to download the data only if it is not available.
#'
#' @return Named list. The list has entries: "available": vector of strings. The names of the files that are available in the local file cache. You can access them using get_optional_data_file(). "missing": vector of strings. The names of the files that this function was unable to retrieve.
#'
#' @export
download_optional_data <- function() {
  ...
  cfiles = pkgfilecache::ensure_files_available_from_manifest(
    pkg_info, manifest, base_url = "https://your.server/yourpackage/"
  );
  cfiles$file_status = NULL;
  return(cfiles);
}
...
Packages that use pkgfilecache can be submitted to CRAN, but keep the [CRAN package policy](https://cran.r-project.org/web/packages/policies.html) in mind:

* **Document the intellectual property** of the downloadable files, using the standard ways in R (a GitHub README is not enough). The policy requires that the ownership of all components, *including anything downloaded during use*, is clear and unambiguous.
* **Do not download data in package code** at install or load time. Follow the interface suggestion above: let users trigger downloads via your API.
* **Unit tests must not write to the user directory on CRAN servers.** Write downloads to a temp dir (`options(pkgfilecache.use_tempdir = TRUE)`) and skip tests that need external data on CRAN (`testthat::skip_on_cran()`). Test downloads on your CI provider instead, and limit CRAN unit tests to data that can be generated in test code.

Several packages on CRAN already use pkgfilecache this way.
